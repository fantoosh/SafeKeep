...
def pgenerate():
    ...
    if request.method == "POST":
        ...
        if length >= 8 and length <= 40:
            password = pgen(
                length, asciiFilter, lowerFilter, upperFilter, puncFilter, digitsFilter
            )

            return render_template("pgenerator.html", password=password)

    password = pgen(
        int(13), asciiFilter, lowerFilter, upperFilter, puncFilter, digitsFilter
    )
    return render_template(
        "pgenerator/pgenerator.html", password=password
    )  # set 13 as the default length of generated password
